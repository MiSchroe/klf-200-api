...
import 'mocha';
import { GW_STATUS_REQUEST_NTF, ParameterActive, StatusOwner, RunStatus, StatusReply, StatusType, CommandOriginator } from "../../src";

describe("KLF200-API", function() {
    describe("GW_STATUS_REQUEST_NTF", function() {
        [StatusType.RequestTargetPosition, StatusType.RequestCurrentPosition, StatusType.RequestRemainingTime].forEach(statusType => {
            describe(`StatusType = ${StatusType[statusType]}`, function() {
                const data = Buffer.from([17, 0x03, 0x07,
                    // Session ID
                    0x47, 0x11,
                    // Status Owner
                    0x02,
                ...
                describe("Constructor", function() {
                    ...
                    it("should return the parameter data", function() {
                        const result = new GW_STATUS_REQUEST_NTF(data);
                        expect(result.ParameterData).to.be.instanceOf(Array).with.deep.members([
                            {
                                ID: 0,
                                Value: 0xC400
                            },
        ...
        describe(`StatusType = ${StatusType[statusType]}`, function() {
            const data = Buffer.from([21, 0x03, 0x07,
                // Session ID
                0x47, 0x11,
                // Status Owner
                0x02,
