...
import 'mocha';
import { GW_GET_SCENE_INFORMATION_CFM, GW_COMMON_STATUS } from "../../src";

describe("KLF200-API", function() {
    describe("GW_GET_SCENE_INFORMATION_CFM", function() {
        describe("Constructor", function() {
            it("should create without error", function() {
                const data = Buffer.from([0x05, 0x04, 0x10, 0x00, 42]);
                expect(() => new GW_GET_SCENE_INFORMATION_CFM(data)).not.to.throw();
            });

            it("should return the scene ID", function() {
                const data = Buffer.from([0x05, 0x04, 0x10, 0x00, 42]);
                const result = new GW_GET_SCENE_INFORMATION_CFM(data);
