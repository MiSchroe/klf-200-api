...
import "mocha";
import { GW_DELETE_GROUP_CFM, GW_COMMON_STATUS } from "../../src";

describe("KLF200-API", function () {
	describe("GW_DELETE_GROUP_CFM", function () {
		describe("Constructor", function () {
			it("should create without error", function () {
				const data = Buffer.from([0x05, 0x02, 0x26, 0x00, 42]);
				expect(() => new GW_DELETE_GROUP_CFM(data)).not.to.throw();
			});

			it("should return the group ID", function () {
				const data = Buffer.from([0x05, 0x02, 0x26, 0x00, 42]);
				const result = new GW_DELETE_GROUP_CFM(data);
