...
import 'mocha';
import { GW_PASSWORD_ENTER_CFM, GW_COMMON_STATUS } from "../../src";

describe("KLF200-API", function() {
    describe("GW_PASSWORD_ENTER_CFM", function() {
        describe("Constructor", function() {
            it("should create without error", function() {
                const data = Buffer.from([0x04, 0x30, 0x01, 0x00]);
                expect(() => new GW_PASSWORD_ENTER_CFM(data)).not.to.throw;
            });

            it("should return the status", function() {
                const data = Buffer.from([0x04, 0x30, 0x01, 0x00]);
                const result = new GW_PASSWORD_ENTER_CFM(data);
