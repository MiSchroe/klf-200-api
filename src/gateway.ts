import { GW_GET_NETWORK_SETUP_CFM } from "./KLF200-API/GW_GET_NETWORK_SETUP_CFM.js";
import { GW_GET_NETWORK_SETUP_REQ } from "./KLF200-API/GW_GET_NETWORK_SETUP_REQ.js";
import { GW_GET_PROTOCOL_VERSION_CFM } from "./KLF200-API/GW_GET_PROTOCOL_VERSION_CFM.js";
...
import { GW_GET_VERSION_CFM, SoftwareVersion } from "./KLF200-API/GW_GET_VERSION_CFM.js";
...
import { GW_RTC_SET_TIME_ZONE_CFM } from "./KLF200-API/GW_RTC_SET_TIME_ZONE_CFM.js";
...
export class Gateway {
	...
	async changePasswordAsync(oldPassword: string, newPassword: string): Promise<boolean> {
		try {
			const passwordChanged: GW_PASSWORD_CHANGE_CFM = <GW_PASSWORD_CHANGE_CFM>(
				await this.connection.sendFrameAsync(new GW_PASSWORD_CHANGE_REQ(oldPassword, newPassword))
			);
	...
	async getVersionAsync(): Promise<{
		...
		try {
			const versionInformation = <GW_GET_VERSION_CFM>(
				await this.connection.sendFrameAsync(new GW_GET_VERSION_REQ())
			);
			return {
	...
	async getProtocolVersionAsync(): Promise<{ MajorVersion: number; MinorVersion: number }> {
		try {
			const versionInformation = <GW_GET_PROTOCOL_VERSION_CFM>(
				await this.connection.sendFrameAsync(new GW_GET_PROTOCOL_VERSION_REQ())
			);
			return {
	...
	async setTimeZoneAsync(timeZone: string): Promise<void> {
		try {
			const timeZoneCFM = <GW_RTC_SET_TIME_ZONE_CFM>(
				await this.connection.sendFrameAsync(new GW_RTC_SET_TIME_ZONE_REQ(timeZone))
			);
	...
	async getNetworkSettingsAsync(): Promise<{
		...
		try {
			const networkSettings = <GW_GET_NETWORK_SETUP_CFM>(
				await this.connection.sendFrameAsync(new GW_GET_NETWORK_SETUP_REQ())
			);
			return {
