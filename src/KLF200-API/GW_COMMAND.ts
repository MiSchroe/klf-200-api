...
export class LockTime {
	...
	static lockTimeTolockTimeValue(lockTime: number): number {
		if (lockTime === Infinity) return 255;

		if (lockTime % 30 !== 0) throw new Error("Lock time must be a multiple of 30.");

		if (lockTime < 0 || lockTime > 7650) throw new Error("Lock time out of range.");
	...
	static lockTimeTolockTimeValueForLimitation(lockTime: number): number {
		if (lockTime === Infinity) return 253;

		if (lockTime % 30 !== 0) throw new Error("Lock time must be a multiple of 30.");

		if (lockTime < 0 || lockTime > 7590) throw new Error("Lock time out of range.");
...
export function convertPosition(position: number, typeID: ActuatorType): number {
	if (position < 0 || position > 1) throw new Error("Position value out of range.");
