...
namespace klf200api {
    ...
    export abstract class GW_FRAME {
        protected constructor(readonly Command: GatewayCommand, readonly BufferSize: number) {
            this.buffer = this.AllocBuffer(BufferSize);           // Allocate the buffer
            this.buffer.writeUInt8(BufferSize, 0);                // Write the frame length to the buffer
            this.buffer.writeUInt16BE(Command, 1);                // Write the command to the buffer
        }
        
        /**
         * Allocates a buffer in the right size for the frame.
         * The first bytes contains the buffer length.
         ...
         * @returns {Buffer} Returns a new buffer initialized with zeros.
         * @memberof GW_FRAME
         */
        private AllocBuffer(BufferSize: number): Buffer {
            return Buffer.alloc(BufferSize + C_COMMAND_SIZE + C_BUFFERLEN_SIZE);
        }

        private buffer: Buffer;
        /**
         * Gets the internal data part of the frame buffer.
         *
         * @readonly
         * @protected
         * @type {Buffer}
         * @memberof GW_FRAME
         */
        protected get Data() : Buffer {
            return this.buffer.slice(2);
        }
    }

    export abstract class GW_FRAME_REQ extends GW_FRAME {
    }

    export abstract class GW_FRAME_RCV extends GW_FRAME {
        protected constructor(Buff: Buffer) {
            let bufferSize: number = Buff.length - C_COMMAND_SIZE - C_BUFFERLEN_SIZE;
            let command: number = Buff.readUInt16BE(1);
            super(command, bufferSize);
            // Copy data
            Buff.copy(this.Data, 0, C_COMMAND_SIZE + C_BUFFERLEN_SIZE);
        }
    }

    export abstract class GW_FRAME_CFM extends GW_FRAME_RCV {}

    export abstract class GW_FRAME_NTF extends GW_FRAME_RCV {}

    export interface ITransferProtocol {
        Encode(Data: GW_FRAME_REQ): Buffer;
        Decode(Data: Buffer): GW_FRAME;
    }
